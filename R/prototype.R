...
plot.eode <- function(x, n.grid = 20, scaleL = 1, scaleAH = 1, scaleS = 1,
  ...
  if(length(axis_var_names)==2){
    ...
    ggplot() +
      ...
      xlab(axis_var_names[1])+ylab(axis_var_names[2]) + theme1
    ...
    ggplot() +
      ...
      xlab(axis_var_names) + ylab(NULL) + theme1 +
    ...
    ggplot() +
      ...
      xlab(NULL) + ylab("velocity") + scale_x_continuous(labels = arrows$name) +
      theme1 +theme(axis.text.x = element_text(hjust = 0.8))
  } else{
  ...
  for(i in 1:length(ParaList)){
    ...
    for(j in 1:length(x$system)){
      ...
      if(any(grepl(paste0(var_name, " = "), deparse(fun)))){
        x$system[[j]] <- eval(parse(text = gsub(paste0(var_name, " = \\d+(\\.\\d+)?"),
...
#' pp(list(x = 1, y = 1))
pp <- function(value){
  if(class(value)!="list") stop("argument 'value' should be an object of class 'list'")
...
#' eq2 <- function(x, y, r2 = 1, a21 = 2, a22 = 1) (r2 - a21 * x - a22 * y) * y
#' x <- eode(dxdt = eq1, dydt = eq2)
#' pdata(x, init = data.frame(x = c(10, 20), y = c(5, 15)),
#'          t = c(3, 3),
...
#' eq2 <- function(x, y, r2 = 1, a21 = 2, a22 = 1) (r2 - a21 * x - a22 * y) * y
#' x <- eode(dxdt = eq1, dydt = eq2)
#' pdat <- pdata(x, init = data.frame(x = c(10, 20), y = c(5, 15)),
#'               t = c(3, 3),
#'               lambda = data.frame(z = c(15, 30)),
#'               formula = "z = x + y")
#' pdat[1]
