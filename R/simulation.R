...
plot.pc <- function(x, model_var_label = NULL, model_var_color = NULL){
  ...
  if(is.null(model_var_color)){
    ggplot(df, aes(x = Time, y = Value, color = Varible)) +
      geom_line(size = 0.9) + theme1
  } else{
    ggplot(df, aes(x = Time, y = Value, color = Varible)) +
      geom_line(size = 0.9) + scale_color_manual(values = color_palette) + theme1
  }
}
...
#' pc_plot(pc)
pc_plot <- function(x){
  if(class(x)!="pc") stop("`x` should be an object of 'pc' class.")
  x$t <- NULL
  x$step <- NULL
  theme1 <- theme_bw()+
    theme(axis.text.x=element_text(size=16,angle=0,colour="black"),
...
#' @param phase_curve simulated phase curve. An object of "\code{pc}" class.
    ...
    if(grepl("dt",formula_now)){
      if(grepl("\\[", formula_now)){
        ...
        res <- as.numeric(lapply(0:(length(phase_curve$t)-1), function(step_now){
          ...
          if(start_step<0 || end_step>(length(phase_curve$t)-1)) return(NA)
...
#' x <- eode(dX_Cdt = dX_Cdt, dY_Cdt = dY_Cdt, dX_Adt = dX_Adt, dY_Adt = dY_Adt,
#' constraint = c("X_C>=0","Y_C>=0","X_A>=0","Y_A>=0"))
#' training_data <- pdata(x, init = data.frame(X_A = c(9, 19, 29, 39),
#'                                             Y_A = c(1, 1, 1, 1),
#'                                             X_C = c(5, 5, 5, 5),
#'                                             Y_C = c(0, 0, 0, 0)),
#'                        t = c(3, 3, 3, 3),
  ...
  for(i in 1:length.pdata(pdat)){
    ...
    for(j in 1:ncol(pdat$lambda)){
      ...
      if(!grepl("dt\\[",pred_formula)){ #in case we want the value of a point
        ...
        pred <- lapply(phase_curve, function(component) component[phase_curve$t > round(pred_start_t/step) &
...
print.pcfamily <- function(x){
  ...
  texts <- as.data.frame(do.call(rbind,lapply(pcf, function(xx){
    c(paras=paste(names(xx$grid_var),xx$grid_var,sep=" = ", collapse = ", "),
...
#' eq2 <- function(x, y, r2 = 1, a21 = 2, a22 = 1) (r2 - a21 * x - a22 * y) * y
#' x <- eode(dxdt = eq1, dydt = eq2, constraint = c("x<1","y<1"))
#' plot.pcfamily(eode_sensitivity_proj(x, valueSpace = list(x = c(0.2, 0.3, 0.4), y = 0.1, a11 = 1:3), N=100))
plot.pcfamily <- function(x, model_var_label = NULL, model_var_color = NULL,
  ...
  if(facet_paras){
    if(!is.null(facet_grid_label)){
      ...
      for(grid_var_name in names(x[[1]]$grid_var)){
        df[,grid_var_name] <- factor(paste(
          grid_var_name, df[,grid_var_name], sep = " = "
    ...
    alpha_breaks <- alpha_gradients[c(1, 1+alpha_index_steps,
