...
eode_is_validval <- function(x, value){
  if(class(value)!="pp") stop("'value' should be an object of class 'pp'")
...
#' the movement of the phase point in a single step is smaller than \cde{eps}.
#'
...
eode_get_cripoi <- function(x, init_value, eps = 10e-4, max_step = 0.01, method = c("Newton","GradDesc")){
  ...
  var_range <- lapply(x$variables, function(var){
    c(minval = as.numeric(str_extract_all(x$constraint[grepl(var,x$constraint) & grepl(">",x$constraint)],"\\d+\\.?\\d*")[[1]]),
  ...
  if(method=="Newton"){
    ...
    while(1){
      ...
      if("error"%in%class(res)){
        ...
        print(data.frame(do.call(rbind, lapply(track, function(xx){ xx }))))
